...
export async function GET(req: NextRequest) {
  try {
    ...
    if (studentQuery) {
      ...
      if (!diagnostic) {
        diagnostic = diagnosticReportRepo.getLatestForStudent(studentQuery) || diagnosticDb.getLatestForStudent(studentQuery);
      }
    }
    
    // If not found by student query, check latest report
    if (!diagnostic) {
      const allReports = diagnosticReportRepo.getAll();
      if (subjectQuery && allReports.length > 0) {
        diagnostic = allReports.find((r) => r.subject.toLowerCase().includes(subjectQuery.toLowerCase())) || allReports[0];
      } else {
    ...
    if (!diagnostic) {
      return NextResponse.json({
        success: false,
        message: 'No diagnostic scan found yet.',
      });
    }
    ...
    const responseData = {
      ...
      questions: (diagnostic as any).questions || [],
