...
export default function AnalyzedSheetView({
  ...
  const [viewMode, setViewMode] = useState<'accordion' | 'notebook'>('accordion');

  return (
    <div className="space-y-6 sm:space-y-8 animate-fadeIn">
      {/* View Switcher Bar */}
      {analysisResult && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-2.5 flex items-center justify-between gap-3 shadow-2xs">
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <span>Question-by-Question Accordion</span>
            </button>
            <button
            ...
            >
              ...
              <span>Notebook OCR Working View</span>
            </button>
          </div>

          <div className="hidden sm:flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400 font-mono pr-2">
            <span>{analysisResult.questions?.length || analysisResult.topic_breakdown?.length || 4} Solved Questions</span>
          </div>
        </div>
      )}
